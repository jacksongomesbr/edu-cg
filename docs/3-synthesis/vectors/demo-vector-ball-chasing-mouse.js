class Ball {
  ...
  draw() {
    ...
    drawArrow(v0, this.getVector(), "green", "ball");
  }
  update(x,y) {
    this.x += x;
    this.y += y;
...
function draw() {
  ...
  let diff = p5.Vector.sub(mouse, ball.getVector());
  if (diff.mag() > 1) {
    let diffNorm = p5.Vector.normalize(diff);
    ball.update(diffNorm.x, diffNorm.y);  
  }
  ball.draw();
